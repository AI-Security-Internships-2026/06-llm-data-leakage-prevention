...
def _checkpoint_save(args, run_id: str, level_name: str, oracle: dict,
                     partial_results: list, n_victims_total: int) -> None:
    """Save per-victim checkpoint so a killed run loses at most 1 victim."""
    ckpt_path = Path(args.output_dir) / f"{run_id}.ckpt_{level_name}.json"
    tmp_path  = ckpt_path.with_suffix(".tmp")
    payload   = {
        "run_id": run_id, "level": level_name,
        "n_victims_total": n_victims_total,
        "n_victims_done":  len(partial_results),
        "oracle":          oracle,
...
def _run_real_load_level(level_name: str, n_tenants: int, args,
                         run_id: str = "") -> dict:
    """
    ...
    with loader:
        ...
        for i, vr in enumerate(victims):
            ...
            if run_id:
                _checkpoint_save(args, run_id, level_name, oracle,
                                 results, len(victims))
...
def _generate_figures(payload: dict, fig_dir: Path) -> None:
    ...
    try:
        ...
        xs = [l["n_tenants"] for l in levels]
        level_labels = [l["level"] for l in levels]

        fig, ax = plt.subplots(figsize=(6,4))
        ax.plot(xs, [l["attack"]["asr"] for l in levels], "o-", color="#3a3", linewidth=2)
        for x, lbl, y in zip(xs, level_labels, [l["attack"]["asr"] for l in levels]):
            ax.annotate(lbl, (x, y), textcoords="offset points", xytext=(0,8), fontsize=8)
        ax.set_xlabel("Background tenants (N)"); ax.set_ylabel("ASR")
        ax.set_title("Fig 1 (Issue #5) — ASR vs Load (real vLLM data)")
        ax.set_ylim(0, 1.1)
        ax.axhline(0.8, color="#888", linestyle="--", linewidth=1, label="Target SR=0.80")
        ax.legend(); fig.tight_layout()
        fig.savefig(fig_dir / "issue5_fig1_asr_vs_load.png", dpi=150); plt.close(fig)

        fig, ax = plt.subplots(figsize=(6,4))
        ax.plot(xs, [l["attack"]["median_api_calls"] for l in levels], "o-",
                color="#e55", label="Median queries", linewidth=2)
        ax.plot(xs, [l["attack"]["p95_api_calls"] for l in levels], "^--",
                color="#fa0", label="P95 queries", linewidth=1.5)
        ax.set_xlabel("Background tenants (N)"); ax.set_ylabel("API queries")
        ax.set_title("Fig 2 (Issue #5) — Query Cost vs Load (real vLLM data)")
        ax.legend(); fig.tight_layout()
        fig.savefig(fig_dir / "issue5_fig2_queries_vs_load.png", dpi=150); plt.close(fig)

        fig, ax = plt.subplots(figsize=(6,4))
        t1s = [l["oracle"].get("t1_threshold_ms", None) for l in levels]
        t2s = [l["oracle"].get("t2_threshold_ms", None) for l in levels]
        if all(t is not None for t in t1s):
            ax.plot(xs, t1s, "o-", color="#36c", label="T1 (name gate)", linewidth=2)
            ax.plot(xs, t2s, "s-", color="#c63", label="T2 (condition gate)", linewidth=2)
            ax.set_xlabel("Background tenants (N)"); ax.set_ylabel("Threshold (ms)")
            ax.set_title("Fig 3 (Issue #5) — Calibrated Thresholds vs Load")
            ax.legend(); fig.tight_layout()
            fig.savefig(fig_dir / "issue5_fig3_thresholds_vs_load.png", dpi=150)
        plt.close(fig)

        fig, ax = plt.subplots(figsize=(6,4))
        bg_reqs = [l["loader_stats"].get("total_requests", 0) for l in levels]
        ax.bar(level_labels, bg_reqs, color="#888", alpha=0.8)
        ax.set_xlabel("Load level"); ax.set_ylabel("Background requests sent")
        ax.set_title("Fig 4 (Issue #5) — Background Load Generated per Level")
        fig.tight_layout()
        fig.savefig(fig_dir / "issue5_fig4_background_load.png", dpi=150); plt.close(fig)

        for stale in ["issue5_fig2_roc_vs_load.png", "issue5_fig3_queries_vs_load.png",
                     "issue5_fig4_ttft_vs_load.png"]:
            p = fig_dir / stale
            if p.exists():
                p.unlink()
                print(f"    [removed stale figure: {stale}]")

        print(f"[load_test] Figures (from REAL data) saved → {fig_dir}")
    except ImportError:
        print("[load_test] matplotlib not installed — JSON/table data saved; plots skipped.")
    ...
    for level_name in levels_to_run:
        ...
        if ckpt_path.exists():
            with open(ckpt_path) as fh:
                ckpt = json.load(fh)
            if ckpt.get("n_victims_done", 0) >= ckpt.get("n_victims_total", 1):
                print(f"[load_test] {level_name}: resuming from checkpoint "
                      f"({ckpt['n_victims_done']}/{ckpt['n_victims_total']} victims done)")
                partial  = ckpt["partial_results"]
                n        = len(partial)
                exact    = sum(r["exact_match"] for r in partial)
                calls    = [r["total_api_calls"] for r in partial]
                lo, hi   = _wilson_ci(exact, n)
                all_levels.append({
                    "level": level_name, "n_tenants": n_tenants,
                    "oracle": ckpt["oracle"],
                    "attack": {
                        "asr": round(exact/n, 4), "exact_recovery_count": exact,
                        "n_victims": n, "asr_ci_95_lo": round(lo, 4),
                        "asr_ci_95_hi": round(hi, 4),
                        "median_api_calls": float(np.median(calls)),
                        "p95_api_calls": float(np.percentile(calls, 95)),
                    },
                    "raw_results": partial,
                })
